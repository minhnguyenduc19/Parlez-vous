...
export function PracticeTab({ onSelectTopic, practicedTopics, togglePracticed, topics }: PracticeTabProps) {
  const [hidePracticed, setHidePracticed] = useState(false);
  const [selectedCategory, setSelectedCategory] = useState<string>('All');

  const categories = useMemo(() => {
    const cats = Array.from(new Set(topics.map(t => t.category).filter(Boolean))) as string[];
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
          <label className="flex items-center gap-2 cursor-pointer text-slate-700 font-medium whitespace-nowrap">
            <input
              type="checkbox"
              checked={hidePracticed}
              onChange={(e) => setHidePracticed(e.target.checked)}
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <select
              ...
              className="w-full sm:w-auto bg-slate-50 border border-slate-200 text-slate-700 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent font-medium"
            >
              {categories.map(cat => (
        ...
        <button
          ...
          disabled={displayedTopics.length === 0}
          className="flex items-center justify-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-300 text-white rounded-xl font-medium shadow-sm transition-all active:scale-95 w-full sm:w-auto"
        >
          <Shuffle size={18} />
          Pick Random Topic
        </button>
      ...
      {displayedTopics.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {displayedTopics.map(topic => {
            ...
            return (
              <div key={topic.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 flex flex-col overflow-hidden hover:shadow-md transition-shadow">
                <div 
                ...
                >
                  ...
                  <div className="absolute top-3 left-3 z-10 flex gap-2">
                    ...
                    {topic.category && (
                      <span className="px-2.5 py-1 bg-indigo-600/80 backdrop-blur-md rounded-lg text-xs font-bold text-white border border-indigo-400/30 shadow-sm">
                        {topic.category}
                      </span>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    className={`absolute top-3 right-3 flex-shrink-0 transition-colors drop-shadow-md z-10 ${isPracticed ? 'text-emerald-400' : 'text-white/80 hover:text-white'}`}
                ...
                <div className="p-5 flex flex-col justify-between gap-4 flex-grow">
                  ...
                  <button
                    ...
                    className="flex items-center justify-center gap-2 w-full py-2.5 bg-slate-50 hover:bg-indigo-50 text-indigo-600 rounded-xl font-medium transition-colors border border-slate-200 hover:border-indigo-200 mt-auto"
                  >
                    <Play size={18} />
                    Discuss
                  </button>
