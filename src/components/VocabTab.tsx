...
interface VocabTabProps {
  ...
  topics: Topic[];
  savedData: SavedData[];
  onEditWord: (topicId: number, oldWord: string, newWord: string) => void;
  onDeleteWord: (topicId: number, word: string) => void;
}

export function VocabTab({ lang, topics, savedData, onEditWord, onDeleteWord }: VocabTabProps) {
  const [editingInfo, setEditingInfo] = useState<{ topicId: number, oldWord: string, value: string } | null>(null);
  const exportRef = useRef<HTMLDivElement>(null);

  const handleEditSave = () => {
    if (!editingInfo) return;
    const { topicId, oldWord, value } = editingInfo;
    onEditWord(topicId, oldWord, value);
    setEditingInfo(null);
  };

  const handleDeleteWord = (topicId: number, word: string) => {
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
        ...
        <button
          ...
          disabled={!hasWords}
          className="flex items-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-300 text-white rounded-xl font-medium shadow-sm transition-all active:scale-95"
        >
          <Download size={18} />
          Export to Image
        </button>
      ...
      {!hasWords ? (
        ...
        <div ref={exportRef} className="bg-slate-50 p-4 sm:p-6 rounded-2xl">
          <div className="space-y-6">
            {savedData.filter(d => d.word.length > 0).map(topicData => {
              ...
              return (
                <div key={topicData.text} className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                  ...
                  <ul className="space-y-2">
                    {topicData.word.map((w, idx) => {
                      ...
                      return (
                        <li key={idx} className="flex items-center justify-between group p-2 hover:bg-slate-50 rounded-lg transition-colors">
                          {isEditing ? (
                            <div className="flex items-center gap-2 w-full">
                              <input
                                type="text"
                                value={editingInfo.value}
                                onChange={(e) => setEditingInfo({ ...editingInfo, value: e.target.value })}
                                className="flex-grow p-1.5 px-3 rounded border border-indigo-300 focus:ring-2 focus:ring-indigo-500 outline-none"
                                autoFocus
                            ...
                            <>
                              ...
                              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" data-html2canvas-ignore>
                                <button
                                  onClick={() => setEditingInfo({ topicId: topicData.text, oldWord: w, value: w })}
                                ...
                                <button
                                  onClick={() => handleDeleteWord(topicData.text, w)}
