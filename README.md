...
| :--- | :--- |
| Bcrypt library | To encrypt the password in the Server, before sending and storing in Database, which is safe when it DB get breaches |
| JWT Token | Ensure UX while keeping the SPPI secured. So that the credential is saved in user's browser in a safe, unaccessible way |
| CORS-ORIGIN | To only allow api-access from designated adrress |
| HTTP-ONLY (settings) | To protect users from malware, which access and steal stored cookies |
...
## 🗺 Future Roadmap
- [ ] Implement the connection between my Eco-system (with Lingo-stack)
